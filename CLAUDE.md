...
The History dock footer (`data-role="history-profile"`) is global dock chrome: it shows the process-wide user profile (`AgentProfileStore`, `~/.psx/profile/` — display name capped at 32 chars, PNG-only avatar ≤ 512KB, atomic writes) and opens the singleton Usage panel dialog (`UsagePanelController` + `UsagePanel.tsx`). Profile/usage commands (`profile_get`/`profile_set_name`/`profile_set_avatar`, `usage_report` `cached`/`force`) are intercepted globally in `AgentWorkspaceCoordinator.OnCommand` (like `load_thread`) and answered by the global `agent_profile`/`agent_usage_report` events; the frontend `UsageRequestBroker` matches every reply by `requestId` end-to-end (late/superseded usage replies are dropped) while requestId-less `agent_profile` broadcasts apply through a monotonic `revision` guard in `UsageStore`. Aggregation lives in `AgentUsageService` only: `AgentThreadStore.ReadUsageSnapshot()` enumerates every thread file (never the 100-entry index), each window (`today`/`last7Days`/`last30Days`) deduplicates active threads independently (never summed from the daily series), turns = user messages, the 365-day series only draws the heatmap, cache hit rate = cacheRead / (input + cacheRead + cacheCreate) (zero denominator → "—"). `IAgentUsageSource` on `IAcpAgentProvider` is the sole Provider-layer extension point for exact usage (`ClaudeSessionUsageSource` scans `CLAUDE_CONFIG_DIR ?? ~/.claude` JSONL restricted to PSX session IDs); `providerSection` rendering is purely field-driven (`exactUsage` → model table, `contextSnapshots` → snapshot list excluded from totals, `sources[]` status badges) and no shared C# or frontend code may branch on a provider name. Usage/profile errors render in the panel, never in the conversation.
