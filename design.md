...
- Shell: the WebView owns a permanent 40px full-height activity rail at the left holding global History, create, Theme, and PSX 设置 buttons. Each rail control keeps a 40×40 hit target; the visible state is a 32×32 plate with `--agent-radius-control` (8px): transparent at rest, `--agent-hover` on hover, `--agent-surface-muted` plus an inset hairline when expanded. Do not use a left accent stripe. A 40px workspace chrome row above the pane contents carries the tab strips/nameplates aligned to each visible column. There is no workspace list in the rail and no second WPF tab or theme row.
...
- Layered shell: History is one global left dock (default 280px, draggable 220–420px) opened from the permanent rail and pushes every pane. Its chrome is a three-layer header — visible `history.title` plus refresh, a full-width search with a Search icon, then a full-width provider filter — with 12px padding and 8px gaps that must not wrap at 220px. Plan is a workspace-local card; below 520px it opens as a pane-local overlay from a summary entry.
...
- Plan and History retain separate DOM, scroll position, and selection state. Completed Plan items keep readable text and pair a check marker with completion styling. History rows highlight with a full-row rounded fill — no inset accent stripes. Workspace groups that contain the active session use the short `history.badge.groupActive` label; the open thread uses `Current` / `Open`. Do not reuse those strings across layers, and do not mark the group with a color-only dot.
- Sidebar function icons on the activity rail and History dock (History, Plus, Palette, Settings, Search, Refresh, Folder, FolderOpen) use the Lucide stroke language at `1.75`. Provider brand marks stay local SVGs. Hidden WPF chrome is not migrated on this surface.
