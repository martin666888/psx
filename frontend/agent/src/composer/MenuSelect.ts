...
const CHEVRON_SVG =
  '<svg class="agent-menu-select-chevron" viewBox="0 0 10 6" width="8" height="5" aria-hidden="true" focusable="false"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>';
const CHECK_SVG =
  '<svg viewBox="0 0 12 10" width="11" height="9" aria-hidden="true" focusable="false"><path d="M1 5l3.5 3.5L11 1" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';

export class MenuSelect {
  private static openInstance: MenuSelect | null = null;

  readonly element: HTMLButtonElement;
  private readonly valueSpan: HTMLSpanElement;
  private readonly onSelect: (value: string) => void;
  private readonly ariaName: string;
  private items: MenuSelectItem[] = [];
  ...
  private readonly onDocumentPointerDown = (event: Event) => {
    ...
    if (!target) return;
    if (this.element.contains(target) || this.popup?.contains(target)) return;
  ...
  private readonly onWindowScroll = (event: Event) => {
    const view = this.element.ownerDocument.defaultView;
    if (view && this.popup && event.target instanceof view.Node && this.popup.contains(event.target)) return;
    this.close();
  };

  constructor(onSelect: (value: string) => void, ariaLabel?: string) {
  ...
  setItems(items: MenuSelectItem[], currentValue: string): void {
    this.items = items;
  ...
  setValue(value: string): void {
    ...
    if (this.ariaName) this.element.setAttribute('aria-label', this.ariaName + ': ' + this.valueSpan.textContent);
    if (this.popup) this.renderOptions();
  }

  setDisabled(disabled: boolean): void {
    this.element.disabled = disabled;
    if (disabled) this.close({ refocus: false });
  }
  ...
  open(): void {
    if (this.popup || this.element.disabled || this.items.length === 0) return;
    ...
    const active = this.popup.children[Math.max(0, this.activeIndex)] as HTMLElement | undefined;
    active?.focus();
  }
  ...
  close(options?: { refocus?: boolean }): void {
    ...
    if (MenuSelect.openInstance === this) MenuSelect.openInstance = null;
    this.popup.remove();
    this.popup = null;
    this.activeIndex = -1;
    this.element.setAttribute('aria-expanded', 'false');
  ...
  private renderOptions(): void {
    ...
    this.items.forEach((item, index) => {
      ...
      if (item.value === this.currentValue) option.insertAdjacentHTML('beforeend', CHECK_SVG);
  ...
  private onPopupKeydown(event: KeyboardEvent): void {
    switch (event.key) {
      ...
      case ' ':
        event.preventDefault();
        if (this.activeIndex >= 0) this.select(this.items[this.activeIndex].value);
        break;
      case 'Escape':
        event.preventDefault();
        this.close();
