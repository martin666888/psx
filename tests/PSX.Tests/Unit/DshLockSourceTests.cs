...
public sealed class DshLockSourceTests
{
    ...
    public void Find_ReturnsVerifiedArtifactForBundledVersion()
    {
        ...
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2"));
        WriteEntryFiles(root, "0.1.2-rc.2");

        var source = new BundledDshLockSource(root);
        var result = source.Find("0.1.2-rc.2");

        Assert.AreEqual(DshLockLookup.Found, result.Kind);
        Assert.AreEqual("0.1.2-rc.2", result.Artifact!.Version);
        ...
        Assert.AreEqual(
            FakeDshLockSource.BuildPackageJson("0.1.2-rc.2"),
    ...
    public void Find_MissingVersion_IsNotBundled_AndCatalogSnapshotListsVersions()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2", blocked: "[{ \"version\": \"0.1.2-rc.4\", \"reason\": \"smoke_failed\" }]"));
        WriteEntryFiles(root, "0.1.2-rc.2");

        var source = new BundledDshLockSource(root);
        Assert.AreEqual(DshLockLookup.VersionNotBundled, source.Find("0.1.2-rc.3").Kind);

        var snapshot = source.LoadCatalog();
        Assert.IsTrue(snapshot.Valid);
        CollectionAssert.AreEqual(new[] { "0.1.2-rc.2" }, snapshot.Versions.ToArray());
        CollectionAssert.AreEqual(new[] { "0.1.2-rc.4" }, snapshot.BlockedVersions.ToArray());
    }
    ...
    public void Find_ShaMismatch_IsArtifactCorrupt()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2", lockSha: new string('A', 64)));
        WriteEntryFiles(root, "0.1.2-rc.2");

        var result = new BundledDshLockSource(root).Find("0.1.2-rc.2");
    ...
    public void Find_MissingArtifactFiles_IsArtifactCorrupt()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2"));
        // No locks/0.1.2-rc.2 directory at all.

        Assert.AreEqual(
            DshLockLookup.ArtifactCorrupt,
            new BundledDshLockSource(root).Find("0.1.2-rc.2").Kind);
    }
    ...
    public void Catalog_MissingOrMalformedOrWrongSchema_IsCatalogInvalid()
    {
        ...
        Assert.AreEqual(DshLockLookup.CatalogInvalid, source.Find("0.1.2-rc.2").Kind);
    ...
    public void Catalog_IncompleteEntryPolicy_IsCatalogInvalid()
    {
        ...
        WriteCatalog(
            root,
            BuildCatalogJson("0.1.2-rc.2").Replace(
    ...
    public void Catalog_NonDescendingEntries_AreCatalogInvalid()
    {
        ...
        var root = Root(workspace);
        var newerEntry =
            "{ \"version\": \"0.1.2-rc.4\", \"lockSha256\": \"" + new string('A', 64)
            + "\", \"packageSha256\": \"" + new string('B', 64)
            + "\", \"lockfileVersion\": 3, \"generatedByNpm\": \"10.9.8\", \"dshSri\": \""
            + DshSri.TestIntegrity + "\", \"smokePassed\": true }";
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2", extraEntries: $", {newerEntry}"));
        WriteEntryFiles(root, "0.1.2-rc.2");
    ...
    public void Catalog_MissingOrNonArrayBlockedVersions_IsCatalogInvalid()
    {
        ...
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2").Replace(
    ...
    public void Catalog_UnknownBlockedReason_IsCatalogInvalid()
    {
        ...
        WriteCatalog(root, BuildCatalogJson(
            "0.1.2-rc.2",
            blocked: "[{ \"version\": \"0.1.2-rc.4\", \"reason\": \"someone_felt_like_it\" }]"));
        WriteEntryFiles(root, "0.1.2-rc.2");
    ...
    public void Catalog_EntryAndBlockedOverlap_IsCatalogInvalid()
    {
        ...
        WriteCatalog(root, BuildCatalogJson(
            "0.1.2-rc.2",
            blocked: "[{ \"version\": \"0.1.2-rc.2\", \"reason\": \"smoke_failed\" }]"));
        WriteEntryFiles(root, "0.1.2-rc.2");
    ...
    public void Catalog_DuplicateEntryVersions_AreCatalogInvalid()
    {
        ...
        var root = Root(workspace);
        var sha = FakeDshLockSource.HashText(FakeDshLockSource.BuildLockJson("0.1.2-rc.2", DshSri.TestIntegrity));
        var packageSha = FakeDshLockSource.HashText(FakeDshLockSource.BuildPackageJson("0.1.2-rc.2"));
        WriteCatalog(
            root,
            $$"""
            {
              "schemaVersion": 1,
              "entries": [
                { "version": "0.1.2-rc.2", "lockSha256": "{{sha}}", "packageSha256": "{{packageSha}}", "lockfileVersion": 3, "generatedByNpm": "10.9.8", "dshSri": "{{DshSri.TestIntegrity}}", "smokePassed": true },
                { "version": "0.1.2-rc.2", "lockSha256": "{{sha}}", "packageSha256": "{{packageSha}}", "lockfileVersion": 3, "generatedByNpm": "10.9.8", "dshSri": "{{DshSri.TestIntegrity}}", "smokePassed": true }
              ],
              "blockedVersions": []
            }
    ...
    public void Catalog_MalformedShaOrVersionFields_AreCatalogInvalid()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-rc.2", lockSha: new string('G', 64)));
        WriteEntryFiles(root, "0.1.2-rc.2");
