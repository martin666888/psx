...
public sealed class DshLockSourceTests
{
    ...
    public void Find_ReturnsVerifiedArtifactForBundledVersion()
    {
        ...
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6"));
        WriteEntryFiles(root, "0.1.2-alpha.6");

        var source = new BundledDshLockSource(root);
        var result = source.Find("0.1.2-alpha.6");

        Assert.AreEqual(DshLockLookup.Found, result.Kind);
        Assert.AreEqual("0.1.2-alpha.6", result.Artifact!.Version);
        ...
        Assert.AreEqual(
            FakeDshLockSource.BuildPackageJson("0.1.2-alpha.6"),
    ...
    public void Find_MissingVersion_IsNotBundled_AndCatalogSnapshotListsVersions()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6", blocked: "[{ \"version\": \"0.1.2-alpha.8\", \"reason\": \"smoke_failed\" }]"));
        WriteEntryFiles(root, "0.1.2-alpha.6");

        var source = new BundledDshLockSource(root);
        Assert.AreEqual(DshLockLookup.VersionNotBundled, source.Find("0.1.2-alpha.7").Kind);

        var snapshot = source.LoadCatalog();
        Assert.IsTrue(snapshot.Valid);
        CollectionAssert.AreEqual(new[] { "0.1.2-alpha.6" }, snapshot.Versions.ToArray());
        CollectionAssert.AreEqual(new[] { "0.1.2-alpha.8" }, snapshot.BlockedVersions.ToArray());
    }
    ...
    public void Find_ShaMismatch_IsArtifactCorrupt()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6", lockSha: new string('A', 64)));
        WriteEntryFiles(root, "0.1.2-alpha.6");

        var result = new BundledDshLockSource(root).Find("0.1.2-alpha.6");
    ...
    public void Find_MissingArtifactFiles_IsArtifactCorrupt()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6"));
        // No locks/0.1.2-alpha.6 directory at all.

        Assert.AreEqual(
            DshLockLookup.ArtifactCorrupt,
            new BundledDshLockSource(root).Find("0.1.2-alpha.6").Kind);
    }
    ...
    public void Catalog_MissingOrMalformedOrWrongSchema_IsCatalogInvalid()
    {
        ...
        Assert.AreEqual(DshLockLookup.CatalogInvalid, source.Find("0.1.2-alpha.6").Kind);
    ...
    public void Catalog_IncompleteEntryPolicy_IsCatalogInvalid()
    {
        ...
        WriteCatalog(
            root,
            BuildCatalogJson("0.1.2-alpha.6").Replace(
    ...
    public void Catalog_NonDescendingEntries_AreCatalogInvalid()
    {
        ...
        var root = Root(workspace);
        var newerEntry =
            "{ \"version\": \"0.1.2-alpha.8\", \"lockSha256\": \"" + new string('A', 64)
            + "\", \"packageSha256\": \"" + new string('B', 64)
            + "\", \"lockfileVersion\": 3, \"generatedByNpm\": \"10.9.8\", \"dshSri\": \""
            + DshSri.TestIntegrity + "\", \"smokePassed\": true }";
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6", extraEntries: $", {newerEntry}"));
        WriteEntryFiles(root, "0.1.2-alpha.6");
    ...
    public void Catalog_MissingOrNonArrayBlockedVersions_IsCatalogInvalid()
    {
        ...
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6").Replace(
    ...
    public void Catalog_UnknownBlockedReason_IsCatalogInvalid()
    {
        ...
        WriteCatalog(root, BuildCatalogJson(
            "0.1.2-alpha.6",
            blocked: "[{ \"version\": \"0.1.2-alpha.8\", \"reason\": \"someone_felt_like_it\" }]"));
        WriteEntryFiles(root, "0.1.2-alpha.6");
    ...
    public void Catalog_EntryAndBlockedOverlap_IsCatalogInvalid()
    {
        ...
        WriteCatalog(root, BuildCatalogJson(
            "0.1.2-alpha.6",
            blocked: "[{ \"version\": \"0.1.2-alpha.6\", \"reason\": \"smoke_failed\" }]"));
        WriteEntryFiles(root, "0.1.2-alpha.6");
    ...
    public void Catalog_DuplicateEntryVersions_AreCatalogInvalid()
    {
        ...
        var root = Root(workspace);
        var sha = FakeDshLockSource.HashText(FakeDshLockSource.BuildLockJson("0.1.2-alpha.6", DshSri.TestIntegrity));
        var packageSha = FakeDshLockSource.HashText(FakeDshLockSource.BuildPackageJson("0.1.2-alpha.6"));
        WriteCatalog(
            root,
            $$"""
            {
              "schemaVersion": 1,
              "entries": [
                { "version": "0.1.2-alpha.6", "lockSha256": "{{sha}}", "packageSha256": "{{packageSha}}", "lockfileVersion": 3, "generatedByNpm": "10.9.8", "dshSri": "{{DshSri.TestIntegrity}}", "smokePassed": true },
                { "version": "0.1.2-alpha.6", "lockSha256": "{{sha}}", "packageSha256": "{{packageSha}}", "lockfileVersion": 3, "generatedByNpm": "10.9.8", "dshSri": "{{DshSri.TestIntegrity}}", "smokePassed": true }
              ],
              "blockedVersions": []
            }
    ...
    public void Catalog_MalformedShaOrVersionFields_AreCatalogInvalid()
    {
        ...
        var root = Root(workspace);
        WriteCatalog(root, BuildCatalogJson("0.1.2-alpha.6", lockSha: new string('G', 64)));
        WriteEntryFiles(root, "0.1.2-alpha.6");
