...
public sealed class DshRuntimeLifecycleTests
{
    ...
    public void CreateLaunchSpec_ValidatedUserUpdate_IsAcceptedBeyondSeedVersion()
    {
        ...
        var (runtime, paths) = CreateRuntime(workspace);
        SeedFakeNodeToolchain(paths);
        SeedAuthorizedDshUpdateTree(paths.DshCurrentDirectory, "0.1.2-alpha.6");

        Assert.IsTrue(runtime.IsInstalled());
        Assert.AreEqual("0.1.2-alpha.6", runtime.CurrentVersion);
        Assert.IsNotNull(runtime.CreateLaunchSpec());
    }
    ...
    public void ValidateStagedUpdate_RejectsNonOfficialRegistryLockEntry()
    {
        ...
        SeedAuthorizedDshUpdateTree(paths.DshNextDirectory, "0.1.2-alpha.6", "https://registry.example/dsh.tgz");

        var error = DshWebRuntime.ValidateStagedUpdate(paths.DshNextDirectory, "0.1.2-alpha.6");
    ...
    public void ValidateStagedUpdate_RejectsDependencyWithoutIntegrityMetadata()
    {
        ...
        var (_, paths) = CreateRuntime(workspace);
        SeedAuthorizedDshUpdateTree(paths.DshNextDirectory, "0.1.2-alpha.6");
    ...
    public void ApplyStagedUpdate_FailedCandidateCanRestoreLastKnownGoodCurrent()
    {
        ...
        Assert.IsTrue(runtime.ApplyStagedUpdate("0.1.2-alpha.6"));
        Assert.AreEqual("0.1.2-alpha.6", runtime.CurrentVersion);
    ...
    public async Task CheckForUpdateAsync_UsesRegistryResultWithoutChangingCurrent()
    {
        ...
        locks.Bundle("0.1.2-alpha.6");
        var (runtime, paths) = CreateRuntime(workspace, locks);
        SeedFakeDshTree(paths.DshCurrentDirectory, DshWebRuntime.SeededPackageVersion);
        SeedScriptedNpm(paths, "console.log(JSON.stringify('0.1.2-alpha.6'));\n");
        ...
        Assert.AreEqual("0.1.2-alpha.6", result.AvailableVersion);
        Assert.HasCount(1, result.AvailableVersions);
        Assert.AreEqual("0.1.2-alpha.6", result.AvailableVersions[0].Version);
        Assert.AreEqual(DshWebRuntime.SeededPackageVersion, runtime.CurrentVersion,
    ...
    public async Task CheckForUpdateAsync_UsesVersionsAndDistTagsCatalog()
    {
        ...
        Assert.IsTrue(result.Success, result.ErrorCode ?? string.Empty);
        Assert.IsTrue(result.UpdateAvailable);
        Assert.AreEqual("0.1.2-alpha.7", result.AvailableVersion);
        Assert.HasCount(1, result.AvailableVersions);
        Assert.AreEqual("0.1.2-alpha.7", result.AvailableVersions[0].Version);
    ...
    public async Task StageUpdateAsync_WritesValidatedNextWithoutTouchingCurrent()
    {
        ...
        var result = await runtime.StageUpdateAsync("0.1.2-alpha.6", CancellationToken.None);

        Assert.IsTrue(result.Success, result.ErrorCode ?? string.Empty);
        Assert.AreEqual(DshWebRuntime.SeededPackageVersion, runtime.CurrentVersion);
        Assert.AreEqual("next", File.ReadAllText(paths.DshActivePointerFile));
        Assert.IsNull(DshWebRuntime.ValidateStagedUpdate(paths.DshNextDirectory, "0.1.2-alpha.6"));
    }
    ...
    public async Task TryCommitProcessState_StaleGeneration_DoesNotPublishReady()
    {
        ...
        using var supervisor = new DshWebRuntimeSupervisor(
            runtime, bridge, Path.Combine(workspace.Path, "supervisor"), Path.Combine(workspace.Path, "dsh-workspace"), CreateSettings(workspace, bridge));
    ...
    [TestMethod]
    public void BuildSurfaceScript_SelfGatesOnSerializedOrigin()
    {
        var script = DshSurfaceHostPolicy.BuildSurfaceScript("http://127.0.0.1:4321");
        StringAssert.Contains(script, "http://127.0.0.1:4321");
        StringAssert.Contains(script, "location.origin");
        StringAssert.Contains(script, "psx-dsh-export");
        StringAssert.Contains(script, "chrome.webview.postMessage");
        Assert.IsFalse(script.Contains("parent.postMessage", StringComparison.Ordinal));
    }
    ...
    public async Task Supervisor_ProductionLauncher_ReachesReadyAndStopsProcessTree()
    {
        ...
        using var supervisor = new DshWebRuntimeSupervisor(
            ...
            CreateSettings(workspace, bridge));
    ...
    [TestMethod]
    public async Task Supervisor_BrowserAuthReadyUrl_HealthCheckDoesNotConsumeToken()
    {
        using var workspace = TestWorkspace.Create(
            nameof(Supervisor_BrowserAuthReadyUrl_HealthCheckDoesNotConsumeToken));
        var (runtime, paths) = CreateRuntime(workspace);
        SeedRealProcessToolchain(paths);
        SeedFakeDshTree(paths.DshCurrentDirectory, DshWebRuntime.SeededPackageVersion);
        var workspaceDirectory = Path.Combine(workspace.Path, "dsh-workspace");
        Directory.CreateDirectory(workspaceDirectory);
        var entryPath = Path.Combine(
            paths.DshCurrentDirectory, "node_modules", "@deepseek-ai", "dsh", "lib", "bin.js");
        File.WriteAllText(entryPath, FakeDshAuthServerSource);
        ...
        await TestWorkspace.WaitUntilAsync(
            () => supervisor.State is DshRuntimeState.Ready or DshRuntimeState.Failed,
            TimeSpan.FromSeconds(20),
            "the authenticated launcher did not reach a terminal startup state");

        Assert.AreEqual(DshRuntimeState.Ready, supervisor.State,
            bridge.Events.LastOrDefault().ToString());
        Assert.IsNotNull(supervisor.ReadyUrl);
        Assert.IsTrue(
            supervisor.ReadyUrl.Query.StartsWith("?token=", StringComparison.Ordinal),
            "the in-memory Ready URL keeps the one-time token");
        var origin = DshWebRuntimeSupervisor.ToFrameOrigin(supervisor.ReadyUrl);
        Assert.AreEqual(origin, LastRuntimeStatus(bridge).GetProperty("readyUrl").GetString());
        Assert.IsFalse(
            LastRuntimeStatus(bridge).ToString().Contains("token=", StringComparison.Ordinal),
            "the wire readyUrl must not carry the launch token");

        var hitsPath = Path.Combine(workspaceDirectory, "dsh-token-hits.txt");
        Assert.IsTrue(File.Exists(hitsPath), "the fake server records token hits");
        Assert.AreEqual("0", File.ReadAllText(hitsPath).Trim(),
            "HealthCheck must GET the clean origin, never the token URL");
    ...
    public async Task Supervisor_UserUpdate_StagesRestartsAndCommitsOnlyAfterReady()
    {
        ...
        locks.Bundle("0.1.2-alpha.6");
        var (runtime, paths) = CreateRuntime(workspace, locks);
        SeedViewAndCiServerNpm(paths, "console.log(JSON.stringify('0.1.2-alpha.6'));");
        SeedFakeDshTree(paths.DshCurrentDirectory, DshWebRuntime.SeededPackageVersion);
        ...
        Assert.AreEqual("0.1.2-alpha.6", LastRuntimeStatus(bridge).GetProperty("availableVersion").GetString());
        Assert.AreEqual(
        ...
        await supervisor.UpdateAndRestartAsync("0.1.2-alpha.6");
        ...
        Assert.AreEqual(0, LastRuntimeStatus(bridge).GetProperty("deferredVersions").GetArrayLength());
        Assert.AreEqual(0, LastRuntimeStatus(bridge).GetProperty("blockedVersions").GetArrayLength());
        await supervisor.StopAsync();
    }
    ...
    public async Task Supervisor_UpdateWithVersionOutsideAllowlist_FailsWithoutTouchingCurrent()
    {
        ...
        await supervisor.UpdateAndRestartAsync("0.1.2-alpha.8");
    ...
    public async Task Supervisor_CachedAllowlistVersion_StillAcceptedAfterCatalogWouldHaveGrown()
    {
        ...
        await TestWorkspace.WaitUntilAsync(
            ...
            "baseline ready");
        await supervisor.CheckForUpdateAsync();
        Assert.AreEqual("0.1.2-alpha.7", LastRuntimeStatus(bridge).GetProperty("availableVersion").GetString());

        await supervisor.UpdateAndRestartAsync("0.1.2-alpha.7");

        Assert.AreEqual(DshRuntimeState.Ready, supervisor.State);
        Assert.AreEqual("0.1.2-alpha.7", runtime.CurrentVersion);
        await supervisor.StopAsync();
    }
    ...
    public async Task Supervisor_CancelUpdateWhileDownloadingKeepsCurrentServerAndCandidateAvailable()
    {
        ...
        SeedScriptedNpm(
            ...
            if (process.argv.includes('view')) {
              console.log(JSON.stringify('0.1.2-alpha.6'));
              process.exit(0);
            }
    ...
    public async Task Supervisor_PartialDeferred_AvailableCarriesBothLists()
    {
        ...
        SeedScriptedNpm(
            ...
            """
            console.log(JSON.stringify({
              versions: ['0.1.2-alpha.2', '0.1.2-alpha.6', '0.1.2-alpha.7', '0.1.2-alpha.8'],
              'dist-tags': { latest: '0.1.2-alpha.8' }
            }));
            """);
        var bridge = new RecordingAgentBridgeService();
        using var supervisor = new DshWebRuntimeSupervisor(
            runtime, bridge, Path.Combine(workspace.Path, "supervisor"),
            Path.Combine(workspace.Path, "dsh-workspace"), CreateSettings(workspace, bridge));
        ...
        Assert.AreEqual("0.1.2-alpha.7", status.GetProperty("availableVersions")[0].GetProperty("version").GetString());
        Assert.AreEqual(1, status.GetProperty("deferredVersions").GetArrayLength());
        Assert.AreEqual("0.1.2-alpha.6", status.GetProperty("deferredVersions")[0].GetProperty("version").GetString());
        Assert.AreEqual(1, status.GetProperty("blockedVersions").GetArrayLength());
        Assert.AreEqual("0.1.2-alpha.8", status.GetProperty("blockedVersions")[0].GetProperty("version").GetString());
    }

    [TestMethod]
    public async Task Supervisor_AllNewerDeferredOrBlocked_ReportsRequiresPsxUpdate()
    {
        using var workspace = TestWorkspace.Create(
            nameof(Supervisor_AllNewerDeferredOrBlocked_ReportsRequiresPsxUpdate));
        var locks = new FakeDshLockSource();
        locks.Block("0.1.2-alpha.7");
        var (runtime, paths) = CreateRuntime(workspace, locks);
        SeedAuthorizedDshUpdateTree(paths.DshCurrentDirectory, DshWebRuntime.SeededPackageVersion);
        SeedScriptedNpm(
            paths,
            ...
            console.log(JSON.stringify({
              versions: ['0.1.2-alpha.2', '0.1.2-alpha.6', '0.1.2-alpha.7'],
              'dist-tags': { latest: '0.1.2-alpha.7' }
            }));
            """);
        var bridge = new RecordingAgentBridgeService();
        using var supervisor = new DshWebRuntimeSupervisor(
        ...
        Assert.AreEqual(0, status.GetProperty("availableVersions").GetArrayLength());
        Assert.AreEqual(1, status.GetProperty("deferredVersions").GetArrayLength());
        Assert.AreEqual(1, status.GetProperty("blockedVersions").GetArrayLength());
    }

    [TestMethod]
    public async Task Supervisor_UpdateCommandUnderRequiresPsxUpdate_RejectedKeepsState()
    {
        using var workspace = TestWorkspace.Create(
        ...
        SeedScriptedNpm(
            ...
            """
            console.log(JSON.stringify({
              versions: ['0.1.2-alpha.6', '0.1.2-alpha.7'],
              'dist-tags': { latest: '0.1.2-alpha.7' }
            }));
            """);
        var bridge = new RecordingAgentBridgeService();
        using var supervisor = new DshWebRuntimeSupervisor(
            ...
            Path.Combine(workspace.Path, "dsh-workspace"), CreateSettings(workspace, bridge));
        await supervisor.CheckForUpdateAsync();
        Assert.AreEqual("requires_psx_update", LastRuntimeStatus(bridge).GetProperty("updateState").GetString());
    ...
    {
        File.WriteAllText(entryPath, FakeDshServerSource);
    }

    private const string FakeDshServerSource = """
        const http = require('http');
        const server = http.createServer((_request, response) => {
          response.writeHead(200, { 'content-type': 'text/html' });
          response.end('<!doctype html><title>DeepSeek Harness</title>');
    ...
    private const string FakeDshAuthServerSource = """
        const fs = require('fs');
        const http = require('http');
        const path = require('path');
        const hitsPath = path.join(process.cwd(), 'dsh-token-hits.txt');
        fs.writeFileSync(hitsPath, '0');
        const TOKEN = 'testtokn';
        const server = http.createServer((request, response) => {
          const url = new URL(request.url, 'http://127.0.0.1');
          if (url.searchParams.has('token')) {
            const next = Number(fs.readFileSync(hitsPath, 'utf8')) + 1;
            fs.writeFileSync(hitsPath, String(next));
    ...
    /// <summary>Scripted npm for update flows: `view` prints the given
    /// registry payload, `ci` installs a fake tree whose bin.js is a working
    /// loopback server (so the supervisor's restart reaches Ready). The
    /// lockfile itself comes from the bundled artifact, so ci never rewrites
    /// it.</summary>
    private static void SeedViewAndCiServerNpm(RuntimePaths paths, string viewStatement)
    {
        SeedScriptedNpm(
            paths,
            $$"""
            ...
            const packageDir = path.join(process.cwd(), 'node_modules', '@deepseek-ai', 'dsh');
            fs.mkdirSync(path.join(packageDir, 'lib'), { recursive: true });
            fs.writeFileSync(path.join(packageDir, 'lib', 'bin.js'), {{System.Text.Json.JsonSerializer.Serialize(FakeDshServerSource)}});
            fs.writeFileSync(path.join(packageDir, 'package.json'), JSON.stringify({ version }));
            """);
    }
    ...
    private static (DshWebRuntime Runtime, RuntimePaths Paths) CreateRuntime(
        TestWorkspace workspace, FakeDshLockSource? locks = null)
    {
    ...
    private static PsxEnvironmentSettingsCoordinator CreateSettings(
        TestWorkspace workspace,
        RecordingAgentBridgeService bridge) =>
        new(new PsxEnvironmentSettingsStore(Path.Combine(workspace.Path, "env")), bridge);
