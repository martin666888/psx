...
.DESCRIPTION
    Produces PSX-{version}-win-x64-portable.zip containing:
      ...
      - tools/pi/                            (Pi + pi-acp from the committed seed lock)
...
# Pi and its adapter are shipped together from the accepted exact seed lock.
Write-Host "==> Bundle Pi and pi-acp into staging/tools/pi" -ForegroundColor Cyan
$piTargetDir = Join-Path $StagingDir "tools\pi"
New-Item -ItemType Directory -Path $piTargetDir -Force | Out-Null
foreach ($name in @("package.json", "package-lock.json", ".npmrc")) {
    Copy-Item -LiteralPath (Join-Path $RepoRoot "tools\pi-seed\$name") -Destination $piTargetDir -Force
}
& $nodeExe $npmCli ci --prefix $piTargetDir --omit=dev --include=optional --engine-strict --ignore-scripts --no-audit --no-fund --registry=https://registry.npmjs.org/ | Out-Host
if ($LASTEXITCODE -ne 0) { throw "Bundled Pi npm ci failed (exit $LASTEXITCODE)" }
$piEntries = @()
foreach ($pair in @(@("@earendil-works/pi-coding-agent", "pi"), @("pi-acp", "pi-acp"))) {
    $packageRoot = Join-Path $piTargetDir ("node_modules\" + $pair[0])
    $manifest = Get-Content -Raw -LiteralPath (Join-Path $packageRoot "package.json") | ConvertFrom-Json
    $relativeBin = if ($manifest.bin -is [string]) { $manifest.bin } else { $manifest.bin.($pair[1]) }
    if ([string]::IsNullOrWhiteSpace($relativeBin)) { throw "Pi bundle has no declared bin entry" }
    $entry = [IO.Path]::GetFullPath((Join-Path $packageRoot $relativeBin))
    if (-not $entry.StartsWith([IO.Path]::GetFullPath($packageRoot).TrimEnd('\') + '\', [StringComparison]::OrdinalIgnoreCase) -or -not (Test-Path -LiteralPath $entry -PathType Leaf)) {
        throw "Pi bundle bin entry is invalid"
    }
    $piEntries += $entry
}
& $nodeExe (Join-Path $StagingDir "tools\pi-launcher\launch.mjs") $piEntries[1] $piEntries[0] --smoke | Out-Host
if ($LASTEXITCODE -ne 0) { throw "Bundled Pi version/ACP smoke failed (exit $LASTEXITCODE)" }
Write-Host "    Pi and pi-acp bundled; version and ACP initialize passed."
...
$requiredFiles = @(
    ...
    "tools\pi\package.json",
    "tools\pi\package-lock.json",
    "tools\pi\node_modules\@earendil-works\pi-coding-agent\package.json",
    "tools\pi\node_modules\pi-acp\package.json",
    "tools\pi-seed\package.json",
    "tools\pi-seed\package-lock.json",
    "tools\pi-seed\.npmrc",
    "tools\pi-launcher\launch.mjs",
...
$requiredFiles += @($piEntries | ForEach-Object { $_.Substring($StagingDir.Length).TrimStart('\').Replace('/', '\') })
...
$forbiddenFiles = @(Get-ChildItem -LiteralPath $StagingDir -Recurse -File | Where-Object {
    ...
    $strayNodeModules = ($relative -match '(^|/)node_modules/') -and (-not $relative.StartsWith('tools/node/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $relative.StartsWith('tools/kimi/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $relative.StartsWith('tools/qwen/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $relative.StartsWith('tools/opencode/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $relative.StartsWith('tools/pi/node_modules/', [StringComparison]::OrdinalIgnoreCase))
...
try {
    ...
    $forbiddenEntries = @($entryNames | Where-Object {
        ...
        $strayNodeModules = ($normalized -match '(^|/)node_modules/') -and (-not $normalized.StartsWith('tools/node/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $normalized.StartsWith('tools/kimi/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $normalized.StartsWith('tools/qwen/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $normalized.StartsWith('tools/opencode/node_modules/', [StringComparison]::OrdinalIgnoreCase)) -and (-not $normalized.StartsWith('tools/pi/node_modules/', [StringComparison]::OrdinalIgnoreCase))
