...
- `entries`：经批准可安装的版本，**最新在前**，至多 5 条（`-Keep`），全部 ≥ 兼容下限（`0.1.3-alpha.2`）；当前首次安装 seed 为 `0.1.5-rc.1`。
...
powershell -ExecutionPolicy Bypass -File tools/generate-dsh-lock.ps1 -Version 0.1.5-rc.1 -NoSmoke

# 正式生成（只能在隔离环境，见下方契约）
powershell -ExecutionPolicy Bypass -File tools/generate-dsh-lock.ps1 -Version 0.1.5-rc.1 -Isolated
