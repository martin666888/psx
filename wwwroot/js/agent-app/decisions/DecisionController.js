...
export class DecisionController {
    ...
    get thread() {
        ...
        return panel ? panel.querySelector('[data-role="thread"]') : null;
    }
    get promptNode() {
        const panel = this.host.getPanel(this.workspaceId);
        return panel ? panel.querySelector('[data-role="mode-transition-prompt"]') : null;
    }
    get inputRowNode() {
        const panel = this.host.getPanel(this.workspaceId);
        return panel ? panel.querySelector('[data-role="input-row"]') : null;
    }
    // --- Shared decision card infrastructure ---------------------------------
    /** Faithful port of _decisionButton. */
    decisionButton(label, action, className) {
        ...
        button.addEventListener('click', () => {
            ...
            if (card && card.dataset.decisionState !== 'active')
                return;
            action();
        });
        return button;
    ...
    appendDecision(event, kind) {
        ...
        options.forEach((option) => {
            ...
            const btn = this.decisionButton(optionName, () => {
                if (details.dataset.decisionState !== 'active')
                    return;
                details.open = false;
                if (kind === 'permission') {
    ...
    resolvePermission(event) {
        ...
        if (headerState)
            headerState.textContent = 'Selected';
        const status = card.querySelector('.agent-mode-transition-status');
        const optionName = asString(event.optionName);
        if (status)
            status.textContent = optionName ? 'Selected: ' + optionName : 'Selection recorded.';
        this.clearModeTransitionPrompt(requestId, true);
    }
    ...
    cancelDecision(requestId, text) {
        ...
        if (card.classList.contains('agent-mode-transition')) {
            ...
            const headerState = card.querySelector('.agent-mode-transition-header-state');
            if (headerState)
                headerState.textContent = 'Cancelled';
            const status = card.querySelector('.agent-mode-transition-status');
            if (status)
                status.textContent = text || 'Request cancelled.';
            this.clearModeTransitionPrompt(requestId, true);
            return;
        }
    ...
    appendElicitation(event) {
        ...
        orderElicitationFields(schema).forEach((entry) => {
            const field = this.createElicitationField(entry.name, entry.property, required.has(entry.name), entry.isSupplement);
            controls.push(field);
        ...
        actions.appendChild(this.decisionButton('Continue', () => {
            if (card.dataset.decisionState !== 'active')
                return;
            let valid = true;
            const content = {};
            ...
            this.bridge()?.sendAgentElicitationResponse(requestId, JSON.stringify({ action: 'accept', content }));
            this.disableDecisionCard(card, 'Response sent.');
        }, 'agent-btn-primary'));
        actions.appendChild(this.decisionButton('Decline', () => {
            if (card.dataset.decisionState !== 'active')
                return;
            this.bridge()?.sendAgentElicitationResponse(requestId, JSON.stringify({ action: 'decline' }));
            this.disableDecisionCard(card, 'Declined.');
        }, 'agent-btn-subtle'));
        actions.appendChild(this.decisionButton('Cancel', () => {
            if (card.dataset.decisionState !== 'active')
                return;
    ...
    createElicitationField(name, property, required, isSupplement) {
        ...
        if (property.type === 'boolean') {
            ...
            if (property.default !== undefined && property.default !== null) {
                control.value = String(property.default);
            }
    ...
    createElicitationOptionButton(option, multi) {
        const button = document.createElement('button');
        button.type = 'button';
    ...
    appendModeTransition(event, historical) {
        ...
        if (options.length === 0) {
            ...
            options.forEach((option) => {
                ...
                const semanticClass = decisionOptionClass(option);
                if (semanticClass)
                    button.classList.add(semanticClass);
        ...
        this.host.appendToTimeline(this.workspaceId, card);
        if (key)
            this.modeTransitionCards[key] = card;
        if (requestId)
            this.modeTransitionCards['request:' + requestId] = card;
        if (toolCallId)
            this.modeTransitionCards['tool:' + toolCallId] = card;
        if (pending) {
            this.showModeTransitionPrompt(event, card, interactive);
            if (wasPinned)
                this.host.scrollModeTransitionToStart(this.workspaceId, card);
        }
        else {
            this.host.scrollTimelineToBottom(this.workspaceId);
        }
    ...
    modeTransitionStateLabel(state, active) {
        if (active)
    ...
    modeTransitionStatusText(state, options, selectedOptionId, active) {
        if (active)
            return 'The Agent is waiting for your selection.';
        if (state === 'error')
            return 'The request cannot continue without an ACP response option.';
        if (state === 'selected') {
        ...
        if (interactive) {
            options.forEach((option) => {
                ...
                if (semanticClass)
                    button.classList.add(semanticClass);
                button.addEventListener('click', () => {
                    if (this.activeModeTransitionRequestId !== requestId || !optionId)
                        return;
    ...
    clearModeTransitionPrompt(requestId, restoreFocus) {
        const prompt = this.promptNode;
        const inputRow = this.inputRowNode;
        if (!prompt || !inputRow)
            return;
        if (requestId && this.activeModeTransitionRequestId !== requestId)
            return;
    ...
    interruptModeTransition(text) {
        ...
        const prompt = this.promptNode;
        if (!requestId && (!prompt || prompt.hidden))
            return;
        const thread = this.thread;
        const card = requestId
            ? this.findDecisionCard(requestId)
            : thread
                ? thread.querySelector('.agent-mode-transition[data-decision-state="active"]')
                : null;
        if (card?.classList.contains('agent-mode-transition')) {
            card.dataset.decisionState = 'disabled';
            card.classList.remove('agent-mode-transition-sending');
            ...
            const headerState = card.querySelector('.agent-mode-transition-header-state');
            if (headerState)
                headerState.textContent = 'Interrupted';
            const status = card.querySelector('.agent-mode-transition-status');
            if (status)
                status.textContent = text || 'This request is no longer active.';
        }
        this.clearModeTransitionPrompt(requestId || '', true);
    }
