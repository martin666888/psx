...
export class ModeTransitionPromptController {
    workspaceId;
    host;
    assistantName = 'Agent';
    activeRequestId = '';
    ...
    showPrompt(event, interactive) {
        ...
        if (interactive) {
            for (const option of options) {
                ...
                button.addEventListener('click', () => {
                    if (this.activeRequestId !== requestId || !optionId)
                        return;
        ...
        if (composerHadFocus) {
            this.requestFrame(() => {
                const target = prompt.querySelector('.agent-composer-decision-option:not(:disabled)') ??
                    stopButton;
                target.focus();
            });
        }
    }
    clearPrompt(requestId, restoreFocus) {
        const prompt = this.promptNode;
        if (!prompt)
            return;
        if (requestId && this.activeRequestId && this.activeRequestId !== requestId)
            return;
    ...
    get promptNode() {
        return this.host
            ...
            ?.querySelector('[data-role="mode-transition-prompt"]') ?? null;
    }
    get inputRowNode() {
        return this.host
            .getPanel(this.workspaceId)
            ?.querySelector('[data-role="input-row"]') ?? null;
    }
}
