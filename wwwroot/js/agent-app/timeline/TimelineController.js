...
export class TimelineController {
    ...
    currentRunGroupBody = null;
    currentRunId = null;
    toolCards = {};
    currentToolCardId = null;
    runToolCounts = null;
    historyGroup = null;
    historyGroupBody = null;
    historyToolCounts = null;
    autoScrollPinnedFlag = true;
    scrollListener = null;
    ...
    applyLegacyEvent(type, raw) {
        switch (type) {
            ...
            case 'tool_started':
                ...
                this.createToolCard(asString(raw.toolCallId) || ('local-' + Date.now()), asString(raw.name) || 'Tool', asString(raw.input), asString(raw.summary), 'running');
                break;
            case 'tool_delta': {
            ...
            case 'raw_terminal_fallback':
                this.appendTool('Raw terminal fallback', asString(raw.text) || 'Unsupported interaction requires terminal fallback.', 'fallback');
                break;
            case 'run_failed':
                ...
                break;
            case 'resume_failed':
                this.appendRecovery(asString(raw.message) || asString(raw.text) || (this.assistantName + ' could not resume this session.'), asString(raw.detail));
                break;
            default:
                break;
        }
    ...
    resolveDecisionFromReact(item, option) {
        if (!item.requestId)
            return;
    ...
    removeToolCardForModeTransition(toolCallId) {
        ...
        if (!thread)
            return;
        thread.querySelectorAll('[data-tool-id]').forEach((element) => {
            if (element.dataset.toolId !== toolCallId)
                return;
    ...
    scrollModeTransitionToStart(card) {
        const thread = this.thread;
        if (!thread)
            return;
        this.requestFrame(() => {
    ...
    scrollToBottom() {
        const thread = this.thread;
        if (!this.autoScrollPinnedFlag || !thread)
            return;
    ...
    appendRecovery(message, detail) {
        ...
        if (!thread)
            return;
        let card = thread.querySelector('.agent-recovery');
        if (!card) {
            ...
            actions.appendChild(this.decisionButton('Open terminal', () => this.bridge()?.sendAgentCommand('terminal'), 'agent-btn-subtle'));
    ...
    appendMessage(role, text) {
        ...
        if (role === 'user') {
            ...
            const cleaned = (text || '').replace(/\{"type":"image"[^\}]*\}/g, '\n\n[image attachment]\n\n');
    ...
    maybeCollapseUserMessage(body) {
        const content = body?.querySelector('.agent-message-content');
        if (!body || !content || !content.textContent || !content.textContent.trim())
            return;
        this.requestFrame(() => {
            this.requestFrame(() => {
    ...
    finalizeAssistantMessage(body) {
        ...
        if (!article)
            return;
        if (article.querySelector('.agent-message-actions'))
            return;
        const actions = document.createElement('div');
    ...
    appendMessageAttachments(attachments) {
    ...
    appendThinkingBlock(text, running) {
    ...
    createThinkingBlock(text, running) {
    ...
    updateRunGroupSummary() {
        if (!this.currentRunGroup || !this.runToolCounts)
            return;
    ...
    createToolCard(toolCallId, name, input, summary, state) {
        if (!this.currentRunGroupBody)
            return undefined;
        ...
        if (this.runToolCounts) {
            ...
            if (details.dataset.state === 'running')
                this.runToolCounts.running++;
            this.updateRunGroupSummary();
        }
        if (this.currentRunGroup) {
    ...
    resolveToolCard(event) {
        ...
        const card = this.createToolCard(toolCallId, asString(event.name) || 'Tool', asString(event.input), asString(event.summary) || asString(event.name) || 'Tool output', 'running');
